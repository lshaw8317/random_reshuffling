...
class Symmetric(StochasticOptimizer):
    """
    Symmetric minibatch-based stochastic gradient descent with decreasing or constant learning rate.
    ...
    def __init__(self, steps_per_permutation=None, lr0=None, lr_max=np.inf, lr_decay_coef=0,
                 lr_decay_power=1, it_start_decay=None, batch_size=1, *args, **kwargs):
        super(Symmetric, self).__init__(*args, **kwargs)
        self.steps_per_permutation = steps_per_permutation if steps_per_permutation else math.ceil(self.loss.n/batch_size)
        self.steps_per_permutation=2*self.steps_per_permutation
        self.lr0 = lr0
        self.lr_max = lr_max
        self.lr_decay_coef = lr_decay_coef
        self.lr_decay_power = lr_decay_power
    ...
    def step(self):
        if self.it%self.steps_per_permutation == 0:
            temp=np.random.permutation(self.loss.n)
            self.permutation = np.hstack((temp,np.flip(temp)))
            self.i = 0
            self.sampled_permutations += 1

        start=self.loss.n*(self.i//self.loss.n)
        if self.i<self.loss.n:
            if (self.loss.n-self.i)%self.batch_size==0:
                if min(self.loss.n,self.i+self.batch_size)!=self.i+self.batch_size:
                    raise ValueError('self.i+self.batch_size should be equal to or less than self.loss.n')
                idx_perm = np.arange(self.i,self.i+self.batch_size)
            else:  #first batch smaller
                if self.i!=0:
                    raise ValueError('self.i should be 0')
                idx_perm = np.arange(self.loss.n%self.batch_size)
        else:
            idx_perm=np.arange(self.i,min(2*self.loss.n,self.i+self.batch_size))
        normalization = len(idx_perm)
        
        idx = self.permutation[idx_perm]
        self.i += normalization
        self.i %= 2*self.loss.n #for SO to work, doesn't affect RR
        # since the objective is 1/n sum_{i=1}^n f_i(x) + l2/2*||x||^2
        # any incomplete minibatch should be normalized by batch_size
        self.grad = self.loss.stochastic_gradient(self.x, idx=idx, normalization=normalization)
        denom_const = 1 / self.lr0
    ...
    def init_run(self, *args, **kwargs):
        super(Symmetric, self).init_run(*args, **kwargs)
        if self.lr0 is None:
            self.lr0 = 1 / self.loss.batch_smoothness(batch_size)
